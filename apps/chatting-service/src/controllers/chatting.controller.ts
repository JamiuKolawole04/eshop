...
import { clearUnseenCount, getUnseenCount, redis } from "@packages/redis";
...
export const newConversation = async (
  ...
  try {
    ...
    const existingGroup = await prisma.conversationGroup.findFirst({
      where: {
        isGroup: false,
        participantIds: { hasEvery: [userId, sellerId] },
      },
    });
...
export const fetchMessages = async (
  ...
  try {
    ...
    const hasAccess = conversation.participantIds.includes(String(userId));
    if (!hasAccess) {
      throw new AuthError("Access denied to this conversation");
    }
...
export const fetchSellerMessages = async (
  ...
  try {
    ...
    if (!conversation.participantIds.includes(String(sellerId))) {
      throw new AuthError("Access denied to this conversation");
    }
